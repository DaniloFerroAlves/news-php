...
if($_GET['action'] == 'restore' && $_GET['pid']){      
   $id = intval($_GET['pid']);   
   $query = mysqli_query($conexao, 'UPDATE posts SET Is_Active = 1 WHERE id = ' .$id. ' ');   
}

if($_GET['action'] == 'perdel' && $_GET['presid']){      
   $id = intval($_GET['presid']);   
   $query = mysqli_query($conexao, 'DELETE FROM posts WHERE id = ' .$id. ' ');       
}
?>
<!DOCTYPE html>
<html lang="pt-br">
                           ...
                           <div class="table-responsive">
                              <table class="table table-colored table-centered table-inverse m-0">
                                 ...
                                 <tbody>
                                    <?php
                                       //noticia
                                       $select = mysqli_query($conexao,'SELECT posts.id as postid, posts.PostTitle as title, posts.CategoryID, category.CategoryName as category FROM posts join Category ON posts.CategoryId = category.id WHERE posts.Is_Active = 0');
                                       $news = mysqli_num_rows($select);
                                       if($news === 0){
                                       ?>
                                    <tr>
